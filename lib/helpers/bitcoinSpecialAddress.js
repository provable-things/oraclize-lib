...
var types = require('bitcoinjs-lib/src/types');
...
bitcoin.Transaction.prototype.hashForSignature__ = function (inIndex, prevOutScript, hashType, ourScript) {
    ...
    // serialize and hash
    var buffer = new Buffer(txTmp.__byteLength(false) + 4); //FIXME
    buffer.writeInt32LE(hashType, buffer.length - 4);
    txTmp.__toBuffer(buffer, 0, false); //FIXME
    return bcrypto.hash256(buffer);
};

function getScript(ORACLIZE, ALICE, BOB, conditions, nLockTime_base) {
    var script = [
        bitcoin.opcodes.OP_DEPTH, bitcoin.opcodes.OP_1SUB, bitcoin.opcodes.OP_0NOTEQUAL, bitcoin.opcodes.OP_NOTIF, // 1 elements on the stack, Alice only?
        ALICE.getPublicKeyBuffer(),
        bitcoin.opcodes.OP_CHECKSIGVERIFY, // Alice has signed the whole script
        bitcoin.script.number.encode(nLockTime_base - 30),
    ...
    for (var j = 0; j < conditions.length; j++) {
        ...
        Array.prototype.push.apply(script, nss);
    }
...
var ORACLIZE = bitcoin.ECPair.fromWIF('038ea27103fb646a2cea9eca9080737e0b23640caaaef2853416c9b286b353313e', network);

function BitcoinSpecialAddress(options) {
    //utils.newInstanceCheck(this, BitcoinSpecialAddress);
    ...
    this.contracts.forEach(function (contract, index, array) {
        contract.getDryMarker(function (thisContract, subcontract) {
            ...
            var marker = bcrypto.sha256(subcontractStable);
            conditionMarkers.push(marker);
            if (index === array.length - 1)
                setAddress(self, options.keys, conditionMarkers);
            }
        );
    });
}

function setAddress(instance, keys, conditions) {
    var NOW = 1482337847; //utils.getUnixTime(new Date); // Importance of this?
    var decodedScript = getScript(ORACLIZE, keys[0], keys[1], conditions, NOW);
    var redeemScript = bitcoin.script.compile(decodedScript);
    var scriptPubKey = bitcoin.script.scriptHash.output.encode(bitcoin.crypto.hash160(redeemScript));
    var address = bitcoin.address.fromOutputScript(scriptPubKey, network);
    console.log("P2SH_address= " + address);
    instance.address = address;
    instance.enableLocks(address, NOW, redeemScript);
}

// Locks down the contracts it encompasses from changes
BitcoinSpecialAddress.prototype.enableLocks = function (address, lockTime, redeemScript) {
...
module.exports = BitcoinSpecialAddress;
