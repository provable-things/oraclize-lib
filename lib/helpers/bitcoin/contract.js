...
BitcoinContract.prototype.prepare = function (options, cb) {
    ...
    if (this.cosigner !== options.signer.getAddress())
        throw new Error('Provided signer does not match the contract cosigner');

    if (this.prepared === true)
        throw new Error('Contract is already prepared... please submit it');
    ...
    request('https://bitcoinfees.21.co/api/v1/fees/recommended', function (error, response, body) {
        var feeRate
        if (!error)
            feeRate = JSON.parse(body).halfHourFee; // temporary

        // internal fallback for feeRate
        if (error || isNaN(feeRate)) {
            feeRate = 90 * 2; // current cheapest and fastest rate
            console.log('There was some issue fetching the feeRate... falling back to default rate of ' + feeRate)
        }

        request("https://test-insight.bitpay.com/api/addr/" + self.escrowAddress + "/utxo", function (error, response, body) {
            if (error)
                console.log(error);
            ...
            //tx.setLockTime(self.lockTime.ts);
            ...
            outputs.forEach(function (output) {
                if (output.value > 0) {
                    ...
                    if (!output.address)
                        output.address = options.spareChangeAddress || self.escrowAddress;
            ...
            txRaw.ins.forEach(function (input, index) {
                var signatureHash = txRaw.hashForSignature__(index, redeemScript, hashType, getSubScriptBuffer(redeemScript, self.marker));
...
BitcoinContract.prototype.recoveryPrepare = function (options, cb) {
    // outputX format is {‘value’: 1234, ‘address’: ‘..’}
    // add network?
    var self = this;
    request('https://bitcoinfees.21.co/api/v1/fees/recommended', function (error, response, body) {
        ...
        request("https://test-insight.bitpay.com/api/addr/" + self.escrowAddress + "/utxo", function (error, response, body) {
            ...
            var tx = new bitcoin.TransactionBuilder(options.network);
            tx.setLockTime(self.lockTime.ts+ 200);
            ...
            inputs.forEach(function (input) {
                tx.addInput(input.txid, input.vout, 0xfffffffe);
            });
            ...
            //TODO add option to specify vin
            txRaw.ins.forEach(function (input, index) {
                var signatureHash = txRaw.hashForSignature(index, redeemScript, hashType);
                var sig = options.signer.sign(signatureHash).toScriptSignature(hashType);

                var newSigChunks = [sig];
                var scriptSigChunks = bitcoin.script.decompile(newSigChunks);
                var serializedScriptPubKey = bitcoin.script.compile(redeemScript);
                var redeemScriptSigEncoded = bitcoin.script.compile([].concat(
                    scriptSigChunks,
...
BitcoinContract.prototype.recoverySign = function (options) {
    var self = this;

    var txHex = self.output.actions[options.index].args.raw_tx;
    var txRaw = bitcoin.Transaction.fromHex(txHex);
    ...
    txRaw.ins.forEach(function (input, index) {
        ...
        var redeemScriptChunks = bitcoin.script.decompile(txRaw.ins[index].script);
        var currentSigChunks = redeemScriptChunks.slice(0, -1);

        var newSigChunks = [sig];
        newSigChunks = newSigChunks.concat(currentSigChunks);
        console.log(newSigChunks);
        var scriptSigChunks = bitcoin.script.decompile(newSigChunks);
        var serializedScriptPubKey = bitcoin.script.compile(redeemScript);
        var redeemScriptSigEncoded = bitcoin.script.compile([].concat(
            scriptSigChunks,
